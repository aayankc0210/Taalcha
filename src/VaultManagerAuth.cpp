...
bool VaultManager::setup(const string& password) {
    ...
    string combined = password + salt_hex;
    string hash = hash_password(combined);
    if (hash.empty()) return false;

    sqlite3_stmt* stmt;
    const char* sql = "INSERT OR REPLACE INTO vault_meta (key, value) VALUES (?, ?);";

    auto ins = [&](const char* key, const string& val) -> bool {
...
bool VaultManager::unlock(const string& password) {
    sqlite3_stmt* stmt;

    auto get_meta = [&](const char* key) -> string {
    ...
    string stored_hash = get_meta("master_hash");
    string salt_hex    = get_meta("master_salt");
